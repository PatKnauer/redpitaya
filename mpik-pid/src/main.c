...
#include "generate.h"
#include "pid.h"
#include "fpga_pid.h" // added by Fenske
...
/* changed by Fenske ---------------------------------------------------------*/
	{ /* pid_NN_arst - Automatic Reset PID NN integrator:
	    *    0 - Do not reset integrator
	    *    1 - Reset integrator automatically           */
	    "pid_11_arst", 0, 1, 0, 0, 1 },
	{ /* pid_NN_sp - PID NN Input-master-gain. */
		"pid_11_gain",  1, 1, 0, -10, 10 },
    { /* pid_NN_sp - PID NN set-point in V. */
        "pid_11_sp",  0, 1, 0, -1, 1 }, //-8192, 8191 },
	{ /* pid_NN_kp - PID NN proportional gain Kp as coefficient ------------------------- Fenske */
		"pid_11_kp",  0, 1, 0, -10, 10 },
    { /* pid_NN_ki - PID NN integral gain     Ki in 1/ms. ------------------------------- Fenske */
        "pid_11_ki",  0, 1, 0, -3600, 3600 },
    { /* pid_NN_kd - PID NN derivative gain   Kd in ns. ------------------------------- Fenske */
        "pid_11_kd",  0, 1, 0, -58, 58 },
	{ /* pid_NN_kd - PID NN output limit in V. ------------------------------- Fenske */
		"pid_11_limit_up",  1, 1, 0, 0, 1 }, //8191 },
	{ /* pid_NN_kd - PID NN output limit in V. ------------------------------- Fenske */
		"pid_11_limit_low",  -1, 1, 0, -1, 0 },
	{ /* pid_NN_int_limit - PID NN integral limit in V (symmetrical). ------------- Fenske*/
		"pid_11_int_limit",  1, 1, 0,  0, 1 },
	{ /* pid_NN_kii - SECOND PID NN integral gain     Kii in 1/ms. ------------------------------- Fenske */
		"pid_11_kii",  0, 1, 0, -3600, 3600 },
	{ /* pid_NN_count_ki - Counter for KI (slows integrator down)  ------------------------------- Fenske */
		"pid_11_count_ki",  1, 1, 0, 1, 1000 },
		...
		{ /* pid_NN_arst - Automatic Reset PID NN integrator:
		   *    0 - Do not reset integrator
		   *    1 - Reset integrator automatically           */
			"pid_12_arst", 0, 1, 0, 0, 1 },
		{ /* pid_NN_sp - PID NN Input-master-gain. */
			"pid_12_gain",  1, 1, 0, -10, 10 },
		{ /* pid_NN_sp - PID NN set-point in V. */
			"pid_12_sp",  0, 1, 0, -1, 1 }, //-8192, 8191 },
		{ /* pid_NN_kp - PID NN proportional gain Kp as coefficient ------------------------- Fenske */
			"pid_12_kp",  0, 1, 0, -10, 10 },
		{ /* pid_NN_ki - PID NN integral gain     Ki in 1/ms. ------------------------------- Fenske */
			"pid_12_ki",  0, 1, 0, -3600, 3600 },
		{ /* pid_NN_kd - PID NN derivative gain   Kd in ns. ------------------------------- Fenske */
			"pid_12_kd",  0, 1, 0, -58, 58 },
		{ /* pid_NN_kd - PID NN output limit in V. ------------------------------- Fenske */
			"pid_12_limit_up",  1, 1, 0, 0, 1 }, //8191 },
		{ /* pid_NN_kd - PID NN output limit in V. ------------------------------- Fenske */
			"pid_12_limit_low",  -1, 1, 0, -1, 0 },
		{ /* pid_NN_int_limit - PID NN integral limit in V (symmetrical). ------------- Fenske*/
			"pid_12_int_limit",  1, 1, 0,  0, 1 },
		{ /* pid_NN_kii -  SECOND PID NN integral gain     Kii in 1/ms. ------------------------------- Fenske */
			"pid_12_kii",  0, 1, 0, -3600, 3600 },
		{ /* pid_NN_count_ki - Counter for KI (slows integrator down)  ------------------------------- Fenske */
			"pid_12_count_ki",  1, 1, 0, 1, 1000 },
		...
		{ /* pid_NN_arst - Automatic Reset PID NN integrator:
		   *    0 - Do not reset integrator
		   *    1 - Reset integrator automatically           */
			"pid_21_arst", 0, 1, 0, 0, 1 },
		{ /* pid_NN_sp - PID NN Input-master-gain. */
			"pid_21_gain",  1, 1, 0, -10, 10 },
		{ /* pid_NN_sp - PID NN set-point in V. */
			"pid_21_sp",  0, 1, 0, -1, 1 }, //-8192, 8191 },
		{ /* pid_NN_kp - PID NN proportional gain Kp as coefficient ------------------------- Fenske */
			"pid_21_kp",  0, 1, 0, -10, 10 },
		{ /* pid_NN_ki - PID NN integral gain     Ki in 1/ms. ------------------------------- Fenske */
			"pid_21_ki",  0, 1, 0, -3600, 3600 },
		{ /* pid_NN_kd - PID NN derivative gain   Kd in ns. ------------------------------- Fenske */
			"pid_21_kd",  0, 1, 0, -58, 58 },
		{ /* pid_NN_kd - PID NN output limit in V. ------------------------------- Fenske */
			"pid_21_limit_up",  1, 1, 0, 0, 1 }, //8191 },
		{ /* pid_NN_kd - PID NN output limit in V. ------------------------------- Fenske */
			"pid_21_limit_low",  -1, 1, 0, -1, 0 },
		{ /* pid_NN_int_limit - PID NN integral limit in V (symmetrical). ------------- Fenske*/
			"pid_21_int_limit",  1, 1, 0,  0, 1 },
		{ /* pid_NN_kii - SECOND PID NN integral gain     Kii in 1/ms. ------------------------------- Fenske */
			"pid_21_kii",  0, 1, 0, -3600, 3600 },
		{ /* pid_NN_count_ki - Counter for KI (slows integrator down)  ------------------------------- Fenske */
			"pid_21_count_ki",  1, 1, 0, 1, 1000 },
	...
	{ /* pid_NN_kd - PID NN derivative gain   Kd in ns. ------------------------------- Fenske*/
		"pid_22_kd",  0, 1, 0, -58, 58 },
	{ /* pid_NN_kd - PID NN output limit in V. ------------------------------- Fenske*/
		"pid_22_limit_up",  1, 1, 0, 0, 1 },
	{ /* pid_NN_kd - PID NN output limit in V. ------------------------------- Fenske*/
		"pid_22_limit_low",  -1, 1, 0, -1, 0 },
	{ /* pid_NN_int_limit - PID NN integral limit in V (symmetrical). ------------- Fenske*/
		"pid_22_int_limit",  1, 1, 0,  0, 1 },
	{ /* pid_NN_kii - SECOND PID NN integral gain     Kii in 1/ms. ------------------------------- Fenske*/
		"pid_22_kii",  0, 1, 0, -3600, 3600 },
	{ /* pid_NN_count_ki - Counter for KI (slows integrator down)  ------------------------------- Fenske */
		"pid_22_count_ki",  1, 1, 0, 1, 1000 },

	{ /* out_x_offset - Output x offset. */
		"out_1_offset",  0, 1, 0, -1, 1 },
	{ /* out_x_offset - Output x offset. */
    ...
    /********************************/
    /* Rotary encoder and save settings from here on */
    /********************************/
	/* added by Fenske ---------------------------------------------------------*/

	{ /* rot_encoder - Rotary encoder */
		"rot_encoder", 0, 0, 1, -1000, +1000 },

	{ /* save_settings - Save PID settings. */
		"save_settings",  0, 1, 0, 0, 3 },
	{ /* load_settings - Load PID settings. */
		"load_settings",  0, 1, 0, 0, 3 },

	//patrick - AO settings and piezo control

	/*enable pin - 0 is off, 1 is on, 0.5 is automatic mode */
	{	"AO_enable_pin0", 0, 0, 0, 0, 1 },
	{	"AO_enable_pin1", 0, 0, 0, 0, 1 },
	{	"AO_enable_pin2", 0, 0, 0, 0, 1 },
	{	"AO_enable_pin3", 0, 0, 0, 0, 1 },

	/*voltage for different pins - start value of 0V*/
	{"AO_vol_pin0", 1, 0, 0, 0, 1.8 },
	{"AO_vol_pin1", 1, 0, 0, 0, 1.8 },
	{"AO_vol_pin2", 1, 0, 0, 0, 1.8 },
	{"AO_vol_pin3", 1, 0, 0, 0, 1.8 },	

	/*piezo_drive on/off*/
	{"piezo_enable", 0, 0, 0, 0, 1},

	/*piezo threshold for MEAS_O_CH1 to activate automatic voltage*/
	{"AO_trsh_up", 0.85, 0, 0, -1, 1},	//from -1V to 1V
	{"AO_trsh_down", -0.4, 0, 0, -1, 1},

	//pulse timer and lenght
	{"pulse_timer", 0, 0, 1, 0, 1000000},
	{"pulse_len", 100, 0, 0, 30, 1000},		
	{"prod_pulse", 0, 0, 1, 0, 1},			//tells piezo_pulse to produce a pulse

	//laser lock variables
	{"value0", 0, 0, 1, -1000, +1000 },		//saves the value to compare the deviation with
	{"ll_timer", -1, 0, 1, -1, 5},			//runs through different comparisons
	{"timerlimit", 20, 0, 0, 1, 1000000,},		//number of times value0 gets compared before its reset
	{"laser_lock_value", 0, 0, 1, 0, 100000},		//if >=... laser is locked
	{"ll", 0, 0, 1, 0, 1},				//0 = not locked, 1 = locked
	{"dev_checks", 100, 0, 0, 0, 100000},		//needed checks to confirm "laser is locked"
	{"dev_in_v", 0.05, 0, 0, 0, 1},			//allowed deviation from value0	

	//manual drive mode
	{"piezo_manual", 1, 0, 0, 0, 1},
	{"manual_dir", 0, 0, 0, 0, 1},
	{"manual_go", 0, 0, 0, 0, 1},
	{"manual_timer", 0, 0, 1, 0, 10000000},
	{"manual_nop", 1, 0, 0, 0, 1000},
	
	{"slow_pulses", 0, 0, 0, 0, 1},
...
{
    return (const char *)"PTB-PID with Red Pitaya application.\n";
}

int rp_app_init(void)
{
    fprintf(stderr, "Loading PTB-PID (with scope+gen+pid extensions) version %s-%s.\n", VERSION_STR, REVISION_STR);
...
{
    fprintf(stderr, "Unloading PTB-PID (with scope+gen+pid extensions) version %s-%s.\n", VERSION_STR, REVISION_STR);
...
int rp_set_params(rp_app_params_t *p, int len)
{
    ...
    int AO_params_change = 0;		//patrick - added params change
    ...
    for(i = 0; i < len || p[i].name != NULL; i++) {
        ...
        if(rp_main_params[p_idx].value != p[i].value) {
            ...
            if ( (p_idx >= PARAMS_AWG_PARAMS) && (p_idx < PARAMS_PID_PARAMS) )
                awg_params_change = 1;
            if ( (p_idx >= PARAMS_PID_PARAMS) && (p_idx < PARAMS_AO_PARAMS) )		//patrick - new index range
                pid_params_change = 1;
	    if(p_idx >= PARAMS_AO_PARAMS)			//patrick - logic for AO_params_change
		AO_params_change = 1;
...
//	if(AO_update(&rp_main_params[0]) < 0) {		//patrick - start AO_update if pid_params_change
//	    return -1;
//        }
    }

    if(AO_params_change) {
	if(AO_update(&rp_main_params[0]) < 0) {
	    return -1;
	}
    }
...
int rp_update_meas_data(rp_osc_meas_res_t ch1_meas, rp_osc_meas_res_t ch2_meas)
{
    ...
    rp_main_params[MEAS_P_CH1].value = ch1_meas.p;  // bar graph -------- Fenske
    rp_main_params[MEAS_I_CH1].value = ch1_meas.i;
    rp_main_params[MEAS_D_CH1].value = ch1_meas.d;
    rp_main_params[MEAS_O_CH1].value = ch1_meas.o;

    rp_main_params[MEAS_P_CH2].value = ch2_meas.p;  // bar graph -------- Fenske
    ...
    if(rp_main_params[PIEZO_ENABLE].value == 1){	//patrick - calling piezo drive every ?50ms?
	laser_locked(&rp_main_params[0]);    		//constantly running if piezo drive is enabled
	if(piezo_drive(&rp_main_params[0]) == 1){	
	piezo_pulse(&rp_main_params[0]);
	}
    }
    else{						//patrick - reset the variables for the laser lock
	rp_main_params[VALUE0].value = 0;
	rp_main_params[LL].value = 0;
	rp_main_params[LASER_LOCK_VALUE].value = 0;
	rp_main_params[LL_TIMER].value = -1;
    }
...
/*----------------------------------------------------------------------------*/
/**
 * @brief Converts counts to float
 *
 * Function is used to publish captured data to external world in user units.
 * This is needed to get negative values too.
 * Calculation is based on maximal value.
 *
 * @param[in] cnts           Captured Signal Value, expressed in ADC counts
 * @param[in] max_int		 Maximal value of float
 * @retval    float          Value, expressed in user units [V]
 */
float cnv_cnt_to_float(int cnts, float max_int)  //----------------------------------------------------------- Fenske
{
    int m;
    float ret_val;

...
void reset_pids(void)
{
    if (g_pid_reg) {
        ...
        for (i = 0; i < NUM_OF_PIDS; i++) {
        	g_pid_reg->pid[i].gain = 100;
            g_pid_reg->pid[i].setpoint = 0;
            g_pid_reg->pid[i].kp = 0;
            g_pid_reg->pid[i].ki = 0;
            g_pid_reg->pid[i].kd = 0;
            g_pid_reg->pid[i].limit_up = 8191; // --------------------------------------------------- Fenske
            g_pid_reg->pid[i].limit_low = -8192; // --------------------------------------------------- Fenske
            g_pid_reg->pid[i].int_limit = 8191; // --------------------------------------------------- Fenske
            g_pid_reg->pid[i].kii = 0; // --------------------------------------------------- Fenske
            g_pid_reg->pid[i].count_ki = 1; // --------------------------------------------------- Fenske
        }

        //g_pid_reg->configuration = 0xf;
        //g_pid_reg->configuration = 0; both versions dont't clear the output.
        //g_pid_reg->configuration = 0x3; // added by Fenske (should clear the output ---------------------------------
        g_pid_reg->configuration = 0xF; // added by Fenske (should clear the output with 4 pids -----------------------

        g_pid_reg->out_1_offset = 0; // Reset output 1 offset value
        g_pid_reg->out_2_offset = 0; // Reset output 1 offset value

...
int fpga_awg_init(void)
{
	...
	/* Set FPGA AWG DAC offset. Read from EEprom */												//------------------ Mattias
	/* You can write calibration values to eeprom with the command line tool like this: */
	/* "calib -r | awk '{$9=-20}1' | calib -w"   '9' for ch_1*/
	g_awg_reg->cha_scale_off = rp_main_calib_params.be_ch1_dc_offs << 16;
	g_awg_reg->chb_scale_off = rp_main_calib_params.be_ch2_dc_offs << 16;

	/* Turn the generator on to set DAC offset*/												//------------------ Mattias
	g_awg_reg->state_machine_conf = 0x00000000;

...
#include "worker.h"
#include "fpga.h"
#include "pid.h"  // bar graph ---------------- Fenske
...
void *rp_osc_worker_thread(void *args)
{
    ...
    while(1) {
        ...
        if(!long_acq || long_acq_idx == 0) {
            ...
            /* converts CNT to V like the input   -------------------- Fenske */
            pid_update_meas_output(&ch1_meas, 1);   // bar graph ---------------- Fenske
            pid_update_meas_output(&ch2_meas, 2);
            rp_osc_meas_output(&ch1_meas, ch1_max_adc_v); // bar graph --------------------- Fenske
            rp_osc_meas_output(&ch2_meas, ch2_max_adc_v);
...
/*----------------------------------------------------------------------------------*/
int rp_osc_meas_output(rp_osc_meas_res_t *ch_meas, float adc_max_v) // bar graph --------------------------- Fenske
{
	float ch1_user_dc_off = -0.022;// depends on RedPitaya-Hardware (for mine: -0.022; for Tobias': -0.017;
	int cnts_meas_p = ch_meas->p;
	int cnts_meas_i = ch_meas->i;
	int cnts_meas_d = ch_meas->d;
	int cnts_meas_o = ch_meas->o;

	ch_meas->p = osc_fpga_cnv_cnt_to_v(cnts_meas_p, adc_max_v,
	                                   	   rp_calib_params->fe_ch1_dc_offs,
										   ch1_user_dc_off) ;
	ch_meas->i = osc_fpga_cnv_cnt_to_v(cnts_meas_i, adc_max_v,
		                                   rp_calib_params->fe_ch1_dc_offs,
		                                   ch1_user_dc_off);
	ch_meas->d = osc_fpga_cnv_cnt_to_v(cnts_meas_d, adc_max_v,
		                                   rp_calib_params->fe_ch1_dc_offs,
		                                   ch1_user_dc_off);
	ch_meas->o = osc_fpga_cnv_cnt_to_v(cnts_meas_o, adc_max_v,
		                                   rp_calib_params->fe_ch1_dc_offs,
		                                   ch1_user_dc_off);
    return 0;
}
